...
def gconnect():
    ...
    try:
        ...
        response = \
            make_response(json.dumps('Failed upgrade the authorization.'), 401)
        response.headers['Content-Type'] = 'application/json'
        return response
    ...
    if result['user_id'] != gplus_id:
        response = \
            make_response(json.dumps("User ID doesn't match user ID."), 401)
        response.headers['Content-Type'] = 'application/json'
        return response
...
def gdisconnect():
    ...
    if result['status'] == '200':
        ...
        response = \
            make_response(json.dumps('Failed revoke token for user.', 400))
        response.headers['Content-Type'] = 'application/json'
        return response
...
def newMenuItem(restaurant_id):
    ...
    if login_session['user_id'] != restaurant.user_id:
        return "<script>function myFunction() {alert('You dont have \
        permission!!! Please create your own restaurant.');}\
          </script><body onload='myFunction()''>"
    if request.method == 'POST':
        newItem = MenuItem(
